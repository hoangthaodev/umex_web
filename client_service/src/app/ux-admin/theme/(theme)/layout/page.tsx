...
import ThemeMode from '@/components/admin/theme/theme/ThemeMode'
import { layoutModeMap } from '@/lib/styleMap'
    ...
    <div className='p-2 flex flex-col gap-4'>
      ...
      <div className='flex flex-col gap-2'>
        ...
        <div className='flex flex-wrap text-xs border border-gray-400'>
          {
            Object.entries(layoutModeMap).map(([key, val], index) => {
              return (
                <label
                  onClick={() => { setLayoutMode(parseInt(key)) }}
                  key={index}
                  className={`${layoutMode === parseInt(key) ? "bg-blue-400 text-gray-100" : ""} grow text-center px-2 py-1`}>{val}</label>
              )
            })
          }
        </div>
                ...
                <div className='flex flex-col'>
                  ...
                  <div className='flex flex-wrap text-xs border border-gray-400'>
                    <label
                      ...
                      className={`${backgroundRepeat === 1 ? "bg-blue-400 text-gray-100" : ""} grow text-center px-2 py-1`}>Full Size</label>
                    <label
                      onClick={() => { setBackgroundRepeat(2) }}
                      className={`${backgroundRepeat === 2 ? "bg-blue-400 text-gray-100" : ""} grow text-center px-2 py-1`}>Tiled</label>
                  </div>
                </div>
              )
            }
