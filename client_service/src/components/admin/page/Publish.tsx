...
const Publish = ({ isSaving, isChanged, page, status, setStatus, publishYear, setPublishYear, publishMonth, setPublishMonth, publishDay, setPublishDay, handleMoveToTrash, handleSave }: Props) => {
    ...
    <div className='border border-gray-400 rounded-sm'>
      ...
      <div className='flex justify-between p-2 items-center'>
        ...
        <div className='flex items-center gap-2'>
          {
            isSaving && <LuLoaderCircle className='text-gray-500 animate-spin' />
          }
          <button className={`${isChanged ? "bg-blue-600 cursor-pointer" : "bg-blue-400 "} px-2 py-1 rounded-sm text-gray-100`}
            onClick={handleSave}
            disabled={!isChanged}
          >Save</button>
        </div>
